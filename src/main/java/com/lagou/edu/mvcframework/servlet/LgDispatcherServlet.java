...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private Properties properties = new Properties();//读取配置文件
        ...
        for (Map.Entry<String, Object> entry : ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if (aClass.isAnnotationPresent(Security.class) && method.isAnnotationPresent(Security.class)) {
                    ...
                    String[] controllerUserSecurity = security.value();
                    List<String> controllerUserList = Arrays.asList(controllerUserSecurity);

                    Security methodAnnotation = method.getAnnotation(Security.class);
                    String[] methodUserSecurity = methodAnnotation.value();
                    List<String> methodUserList = Arrays.asList(methodUserSecurity);
                    controllerUserList.addAll(methodUserList);
                    securityMap.put(handler, controllerUserList);
                } else if (aClass.isAnnotationPresent(Security.class)) {
                    Security classAnnotation = aClass.getAnnotation(Security.class);
                    String[] classUserList = classAnnotation.value();
                    List<String> classUserName = Arrays.asList(classUserList);
                    securityMap.put(handler, classUserName);
                } else if (method.isAnnotationPresent(Security.class)) {
                    Security methodAnnotation = method.getAnnotation(Security.class);
                    String[] methodUserName = methodAnnotation.value();
                    List<String> methodUserList = Arrays.asList(methodUserName);
                    securityMap.put(handler, methodUserList);
                }
...
//        method.invoke() //
        ...
        //安全验证
        List<String> securityUserList = securityMap.get(handler);
        String username = req.getParameter("username");
        if (securityUserList != null && !securityUserList.contains(username)){
            System.out.println("username:"+username+"无权访问");
